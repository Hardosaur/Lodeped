...
//  (!) Для совместимости с Chrome/Safari заменить block на table-cell
//
require('../settings.php');
require('auth.php');
require('connect.inc');
require('access.inc');
$WINDOW_TITLE = 'Лекарственные назначения';
include('header.inc');
//
...
if (isset($_POST['Lek']))
{
  ...
  if (!isset($_POST['form_id'])) die ('<p style="color: red">Не передан id формы препарата!</p>');
  $form_id=$_POST['form_id'];
  if (! (is_numeric ($lek_id) && is_numeric($form_id))) die ('<p style="color: red">Неверный формат данных!</p>');
  ...
  $query = "insert into leks values (NULL, $lek_id, $form_id, $pat_id, $doctor_id, \"$date\", NULL, \"$lek\",0)";
//  print ($query);
...
function On (id)
{
  ...
  if (el.style.display == '' || el.style.display == 'none') el.style.display='table-cell';
}
function Off (id)
{
  var el = document.getElementById(id);
  if (el && el.style.display == 'table-cell') el.style.display="none";
}
...
// ----------------------------------------------------
function selectT15 (sel) // ввод данных о последовательности курсов
{
  if (sel.selectedIndex == 0)
  {
      On('S15');
      Set (23,')');
      Set(21, ' (1-й курс лечения');
  }
  else
  {
    Off('S15');
...
function clickLek (id) // для исправления текста назначения "по месту"
{
  ...
  if (lek_span.style.display=='table-cell')
  {
      lek_span.style.display='none';
      text_span.style.display='table-cell';
  }
  else
  {
      lek_span.style.display='table-cell';
...
if ($res && $res->num_rows)
{
  while ($row = $res->fetch_object())
  {
      ...
      print ('<td><span style="display: table-cell" id="lek'.$row->id.'"><span onclick="javascript:clickLek('.$row->id.')">'.$row->lek);
...
if ($form_id==12 || $form_id==17 || $form_id==7)
{
...
//
?>
<td id="S3" style="display: table-cell">
Метод применения:<br>
<select id="T5" size="10" onchange="javascript:selectT5(this)">
<?php
if ($form_id == 12 || $form_id == 17 || $form_id == 19 || $form_id == 21) // пригодные для инъекций
...
if ($form_id == 1 || $form_id == 8 || $form_id == 9 || $form_id == 12 || $form_id == 11 || $form_id == 15 || $form_id == 16 ||$form_id == 17 || $form_id == 19 || $form_id == 20 || $form_id == 21) // жидкие препараты
   print ('<option value="5">электрофорез на область</option><option value="6">фонофорез на область</option><option value="7">местно на область</option><option value="8">ингаляторно</option><option value="9">интраназально</option><option value="10">эпибульбарно</option><option value="12">интрамеатально</option><option value="13">внутрь</option>');
if ($form_id == 2 || $form_id == 5 || $form_id == 6 || $form_id == 7 || $form_id == 10) // мази и т.п.
   print ('<option value="7">местно на область</option>');
if ($form_id == 3 || $form_id == 4 || $form_id == 18) // таблетки и т.п.
...
//
print ('<td id="S4" style="display: table-cell">');
...
print ('<td id="S5" style="display: table-cell">Единицы:<br><select size="10" id="Ts" onchange="javascript:selectTs(this)">');
if ($form_id == 8 || $form_id == 9 || $form_id == 12 || $form_id == 15 || $form_id == 17 || $form_id == 19 || $form_id == 20 || $form_id == 21) // жидкие препараты
...
//
-->
<td id="S9" style="display: table-cell">
Сколько раз:<br>
...
<td id="S8" style="display: table-cell">
Кратность приёма:<br>
...
// Схема приёма
//
print ('<td id="S10" style="display: table-cell">');
// как принимать
$res = $db->query ('select * from lek_data where tab_id=9');
if (!$res || $res->num_rows!=1) die ('<p>Не найдена таблица 9! Ошибка: '.$db->error.'</p>');
$row=$res->fetch_object();
$opts = explode (';',$row->list);
print ($row->tab_name.':<br><select size="10" id="T9" onchange="javascript:selectT9(this)">');
print ('<option value="по специальной схеме">(по специальной схеме)</option><option value="по специальной методике">(по специальной методике)</option>');
foreach ($opts as $opt)
{
    print ("\n<option value='$opt'>$opt</option>");
}
...
// когда принимать
print ('<td id="S11" style="display: table-cell">');
$res = $db->query ('select * from lek_data where tab_id=10');
if (!$res || $res->num_rows!=1) die ('<p>Не найдена таблица 10! Ошибка: '.$db->error.'</p>');
$row=$res->fetch_object();
$opts = explode (';',$row->list);
print ($row->tab_name.':<br><select size="10" id="T10" onchange="javascript:selectT10(this)"><option value="длительно">длительно</option>');
foreach ($opts as $opt)
{
    print ("\n<option value='$opt'>$opt</option>");
}
...
<tr>
<td>Указать данные о курсе приёма?</td><td><select id="T11" size="1" onchange="javascript:selectT11(this)">
<option value="1">Да</option>
<option value="2" selected>Нет</option>
</select>
</td>
...
<table id="S14" cellspacing="5" cellpadding="5" border="0" style="display: none">
<tr>
<td>Указать данные о последовательности курсов лечения?</td><td><select id="T15" size="1" onchange="javascript:selectT15(this)">
<option value="1">да</option>
<option value="2" selected>нет</option>
</select>
</td>
<td id="S15" style="display: none" align="right">
<select onChange="javascript:document.getElementById('data21').value=this.options[this.selectedIndex].value;if(this.selectedIndex==0){Clear(22);Off('S16');}else{On('S16')};showCourse();">
<option value=" (1-й курс лечения" selected>1 курс</option>
<option value=" (2-й курс лечения;">2 курс</option>
<option value=" (3-й курс лечения;">3 курс</option>
</select>
<input type="hidden" id="data21" value=""/>
&nbsp;
Интервал:&nbsp;
<select id="S16" onChange="javascript:document.getElementById('data22').value=this.options[this.selectedIndex].value;showCourse();">
<option value="" selected>не указывать</option>
<option value=" через 1 неделю после окончания предыдущего курса">1 неделя</option>
<option value=" через 2 недели после окончания предыдущего курса">2 недели</option>
<option value=" через 4 недели после окончания предыдущего курса">4 недели</option>
<option value=" через 6 недель после окончания предыдущего курса">6 недель</option>
<option value=" через 8 недель после окончания предыдущего курса">8 недель</option>
</select>
<input type="hidden" id="data22" value=""/>
<input type="hidden" id="data23" value=""/>
</td>
</tr></table></p>
...
// Полное предписание
//
print ('<form method="post"><input type="hidden" name="lek_name" value="'.$lek_name.'"><input type="hidden" name="form" value="'.$form_name.'"><input type="hidden" name="form_id" value="'.$form_id.'"><p>Текст предписания: <b>');
print ($form_name);
print (' '.$lek_name.' </b>');
